interface DatabaseHelper<Type> {
  insert(entities: Partial<Type>[]): Promise<void>;
...
export default class Repository <Type> {
  ...
  public async insert(entities: Partial<Type>[]): Promise<void> {
    await this.dbHelper.insert(entities);
  }

  public async insertOne(entity: Partial<Type>): Promise<void> {
    await this.dbHelper.insert([entity]);
  }
